...
const Calculator = () => {
  ...
  const result = useMemo(() => {
    ...
    // Доработка 26: финальная прессовка книги.
    const finalPressingItems: SpecItem[] = (() => {
      if (!fpEnabled || !(circulation > 0)) return [];
      if (!fpRows.length) return [];
      const active = fpRows.filter((r) => r.is_active !== false);
      if (!active.length) return [];
      const shortSide = Math.min(dims.w, dims.h);
      const longSide = Math.max(dims.w, dims.h);
      // Толщина книги ~ толщина блока + крышки (~3 мм запас на крышку).
      const sheetThickness = (() => {
        const direct = Number(sewPaperThicknessOverride);
        if (Number.isFinite(direct) && direct > 0) return direct;
        if (paperThickness.length) {
      ...
      const autoBT = Number(sewBlockThicknessOverride)
        || (sheetThickness > 0 ? (sigPages / 2) * sheetThickness : 0)
        || 20;
      const autoBookThickness = autoBT + 3; // ориентировочный запас на крышку
      ...
      const autoBookWeight = autoBlockWeight > 0 ? autoBlockWeight + 50 : 0; // +крышка
      ...
      const fits = (r: FinalPressingRow) =>
        ...
        && circulation >= r.min_circulation && circulation <= r.max_circulation;
      const wantManual = fpManual || fpFabric
        || circulation < 100 || longSide > 500 || bookThickness > 40;
      const sorted = active.slice().sort((a, b) => a.sort_order - b.sort_order);
      ...
      const largeFmtCoef = longSide > row.large_format_threshold ? row.coef_large_format : 1;
      const nonstdFmtCoef = longSide > 420 ? row.coef_nonstandard_format : 1;
      const smallCircCoef = circulation < row.small_circulation_threshold ? row.coef_small_circulation : 1;
      ...
      const items: SpecItem[] = [];
      if (baseCost > 0) {
        const label = mode === "per_time"
          ? `Финальная прессовка книги — ${FINAL_PRESSING_METHOD_LABEL[row.pressing_method] || row.pressing_method} (по времени, ${loadsCount} загр. × ${loadTime} ч)`
          : `Финальная прессовка книги — ${FINAL_PRESSING_METHOD_LABEL[row.pressing_method] || row.pressing_method} (за изделие)`;
        items.push({
          stage: "postpress",
          name: label,
          quantity: circulation,
      ...
      if (coef !== 1 && baseCost > 0) {
        const delta = baseCost * coef - baseCost;
        items.push({ stage: "postpress", name: `Финальная прессовка — коэф. сложности ×${coef.toFixed(2)}`, quantity: 1, unit: "шт", unitPrice: delta, total: delta });
      }
      if (setup > 0) {
        items.push({ stage: "postpress", name: `Финальная прессовка — приладка`, quantity: 1, unit: "шт", unitPrice: setup, total: setup });
      }
      if (total > raw) {
        items.push({ stage: "postpress", name: `Финальная прессовка — доплата до минимума`, quantity: 1, unit: "шт", unitPrice: total - raw, total: total - raw });
      }
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  {ENDPAPER_PRODUCT_TYPES.has(productType) && (
                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex flex-wrap items-center gap-3">
                        <Checkbox checked={gzEnabled} onCheckedChange={(v) => setGzEnabled(!!v)} id="gz" />
                        <Label htmlFor="gz" className="flex-1 font-medium">Марля</Label>
                        <Select
                          value={gzManualId ?? "auto"}
                          onValueChange={(v) => {
                            setGzManualId(v === "auto" ? null : v);
                            setGzPriceOverride(""); setGzGluePriceOverride(""); setGzCoefOverride("");
                            setGzSetupOverride(""); setGzMinOverride("");
                            setGzWidthOverride(""); setGzHeightOverride(""); setGzSpineWidthOverride("");
                          }}
                          disabled={!gzEnabled || gauzeRows.length === 0}
                        >
                          <SelectTrigger className="w-80"><SelectValue placeholder="Запись справочника" /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="auto">Авто (первая активная запись)</SelectItem>
                            {gauzeRows.filter((r) => r.is_active !== false).map((r) => (
                              <SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      {gzEnabled && (() => {
                        if (!gauzeRows.length) {
                          return <p className="text-[11px] text-destructive">Справочник «Марля» пуст — добавьте записи в Справочниках.</p>;
                        }
                        const active = gauzeRows.filter((r) => r.is_active !== false);
                        const row = (gzManualId && active.find((r) => r.id === gzManualId)) || active.slice().sort((a, b) => a.sort_order - b.sort_order)[0];
                        if (!row) return <p className="text-[11px] text-destructive">Нет активных записей.</p>;
                        const blockHeight = Math.max(dims.w, dims.h);
                        const sheetThickness = (() => {
                          const direct = Number(sewPaperThicknessOverride);
                        ...
                        let matPrice = 0;
                        if (row.calc_mode === "per_meter") {
                          matPrice = gzPriceOverride !== "" ? Number(gzPriceOverride) : row.price_per_meter;
                          matCost = (gzHeight / 1000) * matPrice * circulation;
                          matHint = `${(gzHeight / 1000).toFixed(3)} м × ${matPrice} ₸ × ${circulation}`;
                        } else if (row.calc_mode === "per_item") {
                          matPrice = gzPriceOverride !== "" ? Number(gzPriceOverride) : row.price_per_item;
                          matCost = matPrice * circulation;
                          matHint = `${matPrice} ₸ × ${circulation}`;
                        } else {
                          matPrice = gzPriceOverride !== "" ? Number(gzPriceOverride) : row.price_per_m2;
                          matCost = area * matPrice * circulation;
                          matHint = `${area.toFixed(4)} м² × ${matPrice} ₸ × ${circulation}`;
                        }
                        const gluePrice = gzGluePriceOverride !== "" ? Number(gzGluePriceOverride) : row.glue_price_per_item;
                        const glueCost = gluePrice * circulation;
                        const setup = gzSetupOverride !== "" ? Number(gzSetupOverride) : row.setup_cost;
                        ...
                        return (
                          <div className="space-y-2">
                            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Ширина корешка, мм</Label>
                                <Input type="number" inputMode="decimal" step="0.1"
                                  value={gzSpineWidthOverride === "" ? Number(autoBlockThickness.toFixed(2)) : gzSpineWidthOverride}
                                  onChange={(e) => setGzSpineWidthOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Ширина марли, мм</Label>
                                <Input type="number" inputMode="numeric"
                                  value={gzWidthOverride === "" ? Math.round(spineWidth + row.side_overlap * 2) : gzWidthOverride}
                                  onChange={(e) => setGzWidthOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Высота марли, мм</Label>
                                <Input type="number" inputMode="numeric"
                                  value={gzHeightOverride === "" ? blockHeight + row.height_allowance : gzHeightOverride}
                                  onChange={(e) => setGzHeightOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">
                                  Цена материала, ₸/{row.calc_mode === "per_meter" ? "м" : row.calc_mode === "per_item" ? "шт" : "м²"}
                                </Label>
                                <Input type="number" inputMode="decimal" step="0.01"
                                  value={gzPriceOverride === "" ? matPrice : gzPriceOverride}
                                  onChange={(e) => setGzPriceOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Приклейка, ₸/шт</Label>
                                <Input type="number" inputMode="decimal" step="0.01"
                                  value={gzGluePriceOverride === "" ? row.glue_price_per_item : gzGluePriceOverride}
                                  onChange={(e) => setGzGluePriceOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Коэф. сложности</Label>
                                <Input type="number" inputMode="decimal" step="0.01"
                                  value={gzCoefOverride === "" ? Number(autoCoef.toFixed(3)) : gzCoefOverride}
                                  onChange={(e) => setGzCoefOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Приладка, ₸</Label>
                                <Input type="number" inputMode="decimal"
                                  value={gzSetupOverride === "" ? row.setup_cost : gzSetupOverride}
                                  onChange={(e) => setGzSetupOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Мин. стоимость, ₸</Label>
                                <Input type="number" inputMode="decimal"
                                  value={gzMinOverride === "" ? row.min_cost : gzMinOverride}
                                  onChange={(e) => setGzMinOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div className="flex items-center gap-2 pt-5">
                                <Checkbox id="gz-manual" checked={gzManualGlue} onCheckedChange={(v) => setGzManualGlue(!!v)} />
                                <Label htmlFor="gz-manual" className="text-[12px]">Ручная приклейка</Label>
                              </div>
                            </div>
                            <div className="text-[11px] text-muted-foreground space-y-0.5">
                              <div>
                                Подобрано: <b>{row.name}</b> ({GAUZE_TYPE_LABEL[row.gauze_type] || row.gauze_type}, {row.density} г/м², {GAUZE_CALC_LABEL[row.calc_mode] || row.calc_mode})
                              </div>
                              <div>
                                Корешок <b>{spineWidth.toFixed(1)} мм</b> + заходы по {row.side_overlap} мм → марля <b>{Math.round(gzWidth)}×{Math.round(gzHeight)} мм</b> · площадь <b>{area.toFixed(4)} м²</b> · блок <b>{autoBlockThickness.toFixed(1)} мм</b>
                              </div>
                              <div>
                                Материал: {matHint} = <b>{Math.round(matCost).toLocaleString("ru-RU")} ₸</b>
                                {glueCost > 0 && <> · приклейка <b>{Math.round(glueCost).toLocaleString("ru-RU")} ₸</b></>}
                                {setup > 0 && <> · приладка <b>{setup} ₸</b></>}
                              </div>
                              <div>
                                Коэф.: формат {formatCoef} × толст. {thickCoef} × тяж. {heavyBlockCoef} × ручн. {manualGlueCoef} × дизайн. {designerCoef} = <b>{coef.toFixed(2)}</b>
                              </div>
                              <div>
                                Итог: ({Math.round(baseSum).toLocaleString("ru-RU")}) × {coef.toFixed(2)} = <b>{Math.round(total).toLocaleString("ru-RU")} ₸</b>
                                {minCost > 0 && raw < minCost && <> (доплата до мин. {minCost} ₸)</>}
                              </div>
                              {(outOfThickness || outOfCirc) && (
                            ...
                            <p className="text-[11px] text-muted-foreground">
                              Формула: <code>(материал + приклейка + приладка) × коэф</code>; итог = <code>MAX(расчёт, мин. стоимость)</code>. Размер марли: высота = блок + запас, ширина = корешок + 2×заходы.
                            </p>
                          </div>
                        );
                      })()}
                  ...
                  {ENDPAPER_PRODUCT_TYPES.has(productType) && (
                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex flex-wrap items-center gap-3">
                        <Checkbox checked={prEnabled} onCheckedChange={(v) => setPrEnabled(!!v)} id="pr" />
                        <Label htmlFor="pr" className="flex-1 font-medium">Прессовка блока</Label>
                        <Select
                          value={prManualId ?? "auto"}
                          onValueChange={(v) => {
                            setPrManualId(v === "auto" ? null : v);
                            setPrPriceOverride(""); setPrTimeOverride(""); setPrHourPriceOverride("");
                            setPrCoefOverride(""); setPrSetupOverride(""); setPrMinOverride(""); setPrCalcModeOverride("");
                          }}
                          disabled={!prEnabled || pressingRows.length === 0}
                        >
                          <SelectTrigger className="w-80"><SelectValue placeholder="Запись справочника" /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="auto">Авто (по тиражу/толщине/формату)</SelectItem>
                            {pressingRows.filter((r) => r.is_active !== false).map((r) => (
                              <SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      {prEnabled && (() => {
                        if (!pressingRows.length) {
                          return <p className="text-[11px] text-destructive">Справочник «Прессовка блока» пуст — добавьте записи в Справочниках.</p>;
                        }
                        const active = pressingRows.filter((r) => r.is_active !== false);
                        const sheetThickness = (() => {
                          const direct = Number(sewPaperThicknessOverride);
                        ...
                        const total = minCost > 0 ? Math.max(raw, minCost) : raw;
                        const outOfThickness = autoBlockThickness < row.min_block_thickness || autoBlockThickness > row.max_block_thickness;
                        const outOfWeight = blockWeight > row.max_block_weight;
                        const outOfCirc = circulation < row.min_circulation || circulation > row.max_circulation;
                        return (
                          <div className="space-y-2">
                            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Режим расчёта</Label>
                                <Select value={prCalcModeOverride || row.calc_mode} onValueChange={(v) => setPrCalcModeOverride(v as any)}>
                                  <SelectTrigger><SelectValue /></SelectTrigger>
                                  <SelectContent>
                                    <SelectItem value="per_item">За изделие</SelectItem>
                                    <SelectItem value="per_time">По времени</SelectItem>
                              ...
                              <div>
                                ...
                                <Input type="number" inputMode="decimal" step="0.01"
                                  value={prPriceOverride === "" ? row.price_per_item : prPriceOverride}
                                  onChange={(e) => setPrPriceOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Время, сек/изд</Label>
                                <Input type="number" inputMode="decimal" step="0.1"
                                  value={prTimeOverride === "" ? row.time_per_item_sec : prTimeOverride}
                                  onChange={(e) => setPrTimeOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Цена часа, ₸</Label>
                                <Input type="number" inputMode="decimal"
                                  value={prHourPriceOverride === "" ? row.price_per_hour : prHourPriceOverride}
                                  onChange={(e) => setPrHourPriceOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Коэф. сложности</Label>
                                <Input type="number" inputMode="decimal" step="0.01"
                                  value={prCoefOverride === "" ? Number(autoCoef.toFixed(3)) : prCoefOverride}
                                  onChange={(e) => setPrCoefOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Приладка, ₸</Label>
                                <Input type="number" inputMode="decimal"
                                  value={prSetupOverride === "" ? row.setup_cost : prSetupOverride}
                                  onChange={(e) => setPrSetupOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div>
                                <Label className="text-[11px] text-muted-foreground">Мин. стоимость, ₸</Label>
                                <Input type="number" inputMode="decimal"
                                  value={prMinOverride === "" ? row.min_cost : prMinOverride}
                                  onChange={(e) => setPrMinOverride(e.target.value === "" ? "" : Number(e.target.value))} />
                              </div>
                              <div className="flex items-center gap-2 pt-5">
                                <Checkbox id="pr-manual" checked={prManual} onCheckedChange={(v) => setPrManual(!!v)} />
                                <Label htmlFor="pr-manual" className="text-[12px]">Ручная прессовка</Label>
                              </div>
                              <div className="flex items-center gap-2 pt-5">
                                <Checkbox id="pr-des" checked={prDesignerPaper} onCheckedChange={(v) => setPrDesignerPaper(!!v)} />
                                <Label htmlFor="pr-des" className="text-[12px]">Дизайнерская бумага</Label>
                              </div>
                            </div>
                            <div className="text-[11px] text-muted-foreground space-y-0.5">
                              <div>
                                Подобрано: <b>{row.name}</b> ({PRESSING_TYPE_LABEL[row.pressing_type] || row.pressing_type}, {PRESSING_MACHINE_LABEL[row.machine_type] || row.machine_type})
                              </div>
                              <div>
                                Формат <b>{longSide}×{shortSide} мм</b> · блок <b>{autoBlockThickness.toFixed(1)} мм</b> · вес ≈ <b>{Math.round(blockWeight)} г</b>
                              </div>
                              <div>
                                База: {mode === "per_time"
                                  ? <>{timeSec} с × {circulation} / 3600 × {hourPrice} ₸ = <b>{Math.round(baseCost).toLocaleString("ru-RU")} ₸</b></>
                                  : <>{circulation} × {price} ₸ = <b>{Math.round(baseCost).toLocaleString("ru-RU")} ₸</b></>}
                              </div>
                              <div>
                                Коэф.: толщ. {thickCoef} × формат {formatCoef} × тяж. {heavyBlockCoef} × толст. {thickBlockCoef} × ручн. {manualCoef} × дизайн. {designerCoef} × тираж {smallCircCoef} = <b>{coef.toFixed(2)}</b>
                              </div>
                              <div>
                                Итог: ({Math.round(baseCost).toLocaleString("ru-RU")} × {coef.toFixed(2)}) + {setup} = <b>{Math.round(total).toLocaleString("ru-RU")} ₸</b>
                                {minCost > 0 && raw < minCost && <> (доплата до мин. {minCost} ₸)</>}
                              </div>
                              {(outOfThickness || outOfWeight || outOfCirc) && (
                                <div className="text-warning">
                                  ⚠ {outOfThickness && <>толщина блока вне диапазона {row.min_block_thickness}–{row.max_block_thickness} мм</>}
                                  {outOfWeight && <>; вес блока превышает {row.max_block_weight} г</>}
                                  {outOfCirc && <>; тираж вне диапазона {row.min_circulation}–{row.max_circulation}</>}
                                </div>
                              )}
                            </div>
                ...
                <div className="mt-4 space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                  ...
                  {result && !("error" in result) && result.layout && (
                    <details className="rounded-md border bg-card p-3" open>
                      <summary className="cursor-pointer text-sm font-medium">Раскладка</summary>
                      <div className="mt-3">
                        <LayoutPreview
                          layout={result.layout}
                          productW={dims.w}
